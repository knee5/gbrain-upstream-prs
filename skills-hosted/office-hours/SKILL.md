---
name: office-hours
description: "Startup diagnostic in the YC office-hours style: six forcing questions that expose whether the product has real pull, plus a builder mode for design-partner conversations"
triggers:
  - "office hours"
  - "startup diagnostic in the yc office-hours style"
  - "six forcing questions that expose whether the product has real pull"
  - "plus a builder mode for design-partner conversations"
---
...
1. Read `the project instructions file`, `the project TODO list` (if they exist).
2. Run `git log --oneline -30` and `git diff origin/main --stat 2>/dev/null` to understand recent context.
3. Use Grep/Glob to map the codebase areas most relevant to the user's request.
4. **List existing design docs for this project:**
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   ls -t the local state folder (unavailable here)$SLUG/*-design-*.md 2>/dev/null
   ```
...
Ask these questions **ONE AT A TIME** via a multiple-choice question to the user. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via a multiple-choice question to the user. The goal is to brainstorm and sharpen the idea, not interrogate.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" the local state folder (unavailable here)$SLUG/*-design-*.md 2>/dev/null
```

If matches found, read the matching design docs and surface them:
- "FYI: Related design found — '{title}' by {user} on {date} (branch: {branch}). Key overlap: {1-line summary of relevant section}."
- Ask via a multiple-choice question to the user: "Should we build on this prior design or start fresh?"

This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `the local state folder (unavailable here)`.
...
**Privacy gate:** Before searching, use a multiple-choice question to the user: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use a multiple-choice question to the user to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Emit ONE a multiple-choice question to the user that lists every alternative (A/B and optionally C) as numbered options, using the preamble's a multiple-choice question to the user Format section. The a multiple-choice question to the user call is a tool_use, not prose — write the question text and call the tool.
...
If `$D serve` is not available or fails, fall back to a multiple-choice question to the user:
...
If Codex is available, use a multiple-choice question to the user:
...
and writes via atomic mktemp+mv to `the local state folder (unavailable here)`.
...
```bash
the local helper CLI (unavailable here) --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `your skills folder/gstack/office-hours/sections/design-and-handoff.md` and execute it
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one a multiple-choice question to the user.
