...
description: "Propagate a correction through everything that depends on a wrong fact: find downstream claims, fix them, and leave a visible correction trail instead of a silent edit"
triggers:
  - "correction pipeline"
  - "propagate a correction through everything that depends on a wrong fact"
  - "find downstream claims"
  - "fix them"
  - "and leave a visible correction trail instead of a silent edit"
---
...
```bash
the knowledge base search "<relevant terms>" --limit 10
```

For concept-shaped or synonym-phrased claims, escalate to `(look this up in your connected knowledge base, if you have one)` (LLM expansion recovers phrasings `search` misses). Also grep
the brain repo checkout directly — resolve it once from config:

```bash
BRAIN_DIR=$(the knowledge base config get sync.repo_path)
...
**Important:** on the knowledge base installs these files are RENDERED from the bootstrap
...
```bash
the knowledge base recall <entity-slug>            # facts about the subject, newest first
the knowledge base recall --grep "<claim terms>"   # substring filter when the entity is unclear
```
...
| **FACTS_TABLE_ERROR** | Wrong fact in the the knowledge base facts table | `recall` → `forget <fact-id>` → `remember` the correction |
...
- **BRAIN_ERROR:** Edit the page file in the brain repo. Include
  ...
  `the knowledge base sync` so the DB reflects the fix. (Editing the DB row without the
...
- **SOUL_USER_ERROR:** NEVER edit SOUL.md / USER.md directly — they are
  ...
  ```bash
  the knowledge base bootstrap interview --set KEY "corrected value"   # verbatim, user's words
  the knowledge base bootstrap interview --show                        # read back
  the knowledge base bootstrap interview --status                      # get the confirm hash
  the knowledge base bootstrap interview --confirm <hash>
  the knowledge base bootstrap render --only SOUL.md --force           # repeat per affected file
  ```
...
- **FACTS_TABLE_ERROR:** Expire the wrong row and write the correction with
  provenance:
  ```bash
  the knowledge base recall <entity-slug>                                  # find the fact id
  the knowledge base forget <fact-id>                                      # expire the wrong fact
  the knowledge base remember "<correct fact>" \
...
- **HALLUCINATION:** There is no source to fix. Identify the partial signal
  ...
  ```bash
  the knowledge base remember "WRONG: <what was said>. RIGHT: <what is true>. Guard: <instruction to prevent recurrence>" \
...
the knowledge base search "<wrong claim terms>" --limit 20
```
...
```bash
the knowledge base eval suspected-contradictions   # run the probe
the knowledge base find-contradictions             # read the latest run's findings
```
...
- Propagation is checked (whole-brain grep + `(look this up in your connected knowledge base, if you have one)`)
...
- The contradictions probe (`the knowledge base eval suspected-contradictions` /
  `the knowledge base find-contradictions`) — PROACTIVE intra-brain conflict detection.
