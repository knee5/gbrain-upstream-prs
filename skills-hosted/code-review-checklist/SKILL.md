...
description: "Structured code review of a diff: scope-drift detection, a critical correctness pass, confidence calibration, adversarial review, and fix-first triage"
triggers:
  - "code review checklist"
  - "structured code review of a diff"
  - "scope-drift detection"
  - "a critical correctness pass"
...
1. Read `the project TODO list` (if it exists). Read PR description (`gh pr view --json body --jq .body 2>/dev/null || true`).
   Read commit messages (`git log origin/<base>..HEAD --oneline`).
   **If no PR exists:** rely on commit messages and the project TODO list for stated intent — this is the common case since /review runs before /ship creates the PR.
...
4. Evaluate with skepticism (incorporating plan completion results if available from an earlier step or adjacent section):
   ...
   **MISSING REQUIREMENTS detection:**
   - Requirements from the project TODO list/PR description not addressed in the diff
...
# Compute project slug for the local state folder (unavailable here) lookup
...
2. **the project TODO list:** If it exists, check for items related to this branch or recent dates
...
**Only for discrepancies sourced from plan files** (not commit messages or the project TODO list), log a learning so future sessions know this pattern occurred:

```bash
the local helper CLI (unavailable here) '{
  "type": "pitfall",
...
**Do NOT log learnings from commit-message-derived or the project TODO list-derived discrepancies.** These are informational in the review output but too noisy for durable memory.
...
- **HIGH-impact discrepancies** trigger a multiple-choice question to the user:
  - Show the investigation findings
  - Options: A) Stop and implement missing items, B) Ship anyway + create P1 TODOs, C) Intentionally dropped

This is **INFORMATIONAL** unless HIGH-impact discrepancies are found (then it gates via a multiple-choice question to the user).
...
**No plan file found:** Use commit messages and the project TODO list as fallback sources (see above). If no intent sources at all, skip with: "No intent sources detected — skipping completion audit."
...
`the local state folder (unavailable here)` design doc.
...
If there are ASK items remaining, present them in ONE a multiple-choice question to the user:
...
If 3 or fewer ASK items, you may use individual a multiple-choice question to the user calls instead of batching.
...
2. **FALSE POSITIVE comments:** Present each one via a multiple-choice question to the user:
...
3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no a multiple-choice question to the user needed:
...
Read `the project TODO list` in the repository root (if it exists). Cross-reference the PR against open TODOs:

- **Does this PR close any open TODOs?** If yes, note which items in your output: "This PR addresses TODO: <title>"
- **Does this PR create work that should become a TODO?** If yes, flag it as an informational finding.
- **Are there related TODOs that provide context for this review?** If yes, reference them when discussing related findings.

If the project TODO list doesn't exist, skip this step silently.

---

## Step 5.6: Documentation staleness check

Cross-reference the diff against documentation files. For each `.md` file in the repo root (README.md, ARCHITECTURE.md, CONTRIBUTING.md, the project instructions file, etc.):
...
If GATE is FAIL, use a multiple-choice question to the user:
```
...
```bash
the local helper CLI (unavailable here) '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
