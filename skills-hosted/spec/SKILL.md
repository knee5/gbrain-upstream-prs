---
name: spec
description: "Turn vague intent into a precise, executable spec in phases, with issue quality standards and templates for context, acceptance criteria, testing, rollback, and scope"
triggers:
  - "spec"
---
...
- **1+ matches:** surface them to the user via a multiple-choice question to the user: "Found {N} similar
...
a multiple-choice question to the user: A) edit, B) acknowledge and proceed, C) cancel. **On a PUBLIC repo,
...
bun your skills folder/gstack/lib/redact-audit-log.ts \
...
block; MEDIUM PII/legal/internal confirm via a multiple-choice question to the user; LOW surfaces). Full
...
2. **Exit 2 (MEDIUM)** — a multiple-choice question to the user per finding (cluster identical ids; PUBLIC
...
- **Score <7, iteration 2:** print "Quality gate: {score}/10 (after one
  revision). Codex still flags: {ambiguities}." a multiple-choice question to the user:
  - A) Ship anyway (file at this quality)
  - B) Save draft locally and stop (no issue filed)
  - C) One more revision attempt

Max 3 dispatches total. If still <7 after iter 3, a multiple-choice question to the user same options.
...
`a multiple-choice question to the user`. For open-ended interrogation, ask inline in the chat — the
