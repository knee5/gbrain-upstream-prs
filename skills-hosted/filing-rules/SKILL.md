---
name: filing-rules
description: "The full reference for where a document belongs in a knowledge base: directory meanings, naming, and the decision order when a page could live in two places. Companion to repo-architecture"
triggers:
  - "filing rules"
  - "the full reference for where a document belongs in a knowledge base"
  - "directory meanings"
  - "and the decision order when a page could live in two places"
---
...
**Size routing (automatic via `the knowledge base files upload-raw`):**
...
**Upload command:**
```bash
the knowledge base files upload-raw <file> --page <page-slug> --type <type>
```
...
```bash
the knowledge base files signed-url <storage-path>    # Generate 1-hour signed URL
the knowledge base files restore <dir>                # Download back to local
```
...
The `synthesize` and `patterns` phases of `the knowledge base dream` write to a
...
When writing a `<!--- the knowledge base:takes:begin -->` fence, the **holder** column says
