...
description: "Run a retrospective over a period of work: what shipped, what patterns recurred, what to change. Includes report structure and a compare mode across periods"
triggers:
  - "engineering retro"
  - "run a retrospective over a period of work"
  - "what shipped"
...
cat the local state folder (unavailable here)$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x the local helper CLI (unavailable here) ] && DISCOVER_BIN=the local helper CLI (unavailable here)
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t the local state folder (unavailable here)*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p the local state folder (unavailable here)
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls the local state folder (unavailable here)${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `the local state folder (unavailable here)${today}-${next}.json`:
...
- Do not read the project instructions file or other docs — this skill is self-contained
- On first run (no prior retros), skip comparison sections gracefully
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `the local state folder (unavailable here)` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
