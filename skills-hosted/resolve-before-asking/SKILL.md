...
description: "Exhaust what you can determine yourself before interrupting a human with a question, and when you must ask, ask one decision-shaped question with a recommended default"
triggers:
...
```bash
the knowledge base think "Who is {entity}? What is their relationship to the user? What role do they play? Use all available context — meetings, timeline, imported archives, facts."
```
...
```bash
the knowledge base search "{entity}" --limit 5
the knowledge base get {entity-slug}
```
...
```bash
the knowledge base sources list
the knowledge base query "emails with {entity}" --limit 10
the knowledge base query "meetings with {entity}" --limit 5
```
...
```bash
the knowledge base timeline {entity-slug} --limit 20
the knowledge base backlinks {entity-slug}
the knowledge base graph {entity-slug} --depth 2
```
...
Use the `ask-user` skill for the choice-gate mechanics (2–4
...
- **the `query` skill** — owns the lookup verb mechanics (3-layer
...
- **the `brain-ops` skill** — owns the general read-enrich-write
...
- **the `ask-user` skill** — owns HOW to ask (choice-gate format,
  ...
  hypothesis).
- **the `enrich` skill** — owns creating and updating entity pages
