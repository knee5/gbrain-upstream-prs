...
description: "Write documentation from scratch using the Diataxis framework: partition into tutorial, how-to, reference, and explanation, then write each to its own standard with cross-links"
triggers:
  - "document generate"
  - "write documentation from scratch using the diataxis framework"
  - "partition into tutorial"
  - "reference"
  - "and explanation"
  - "then write each to its own standard with cross-links"
---
...
2. Use a multiple-choice question to the user to confirm scope and ask about documentation target:
...
2. **Read the entry points.** Identify and read:
   - README.md, ARCHITECTURE.md, CONTRIBUTING.md, the project instructions file / AGENTS.md
...
If the plan has more than 5 documents to create, use a multiple-choice question to the user to confirm before proceeding.
...
2. **Update entry-point files.** Add references to new docs in:
   ...
   - the project instructions file / AGENTS.md — add to project structure if relevant
