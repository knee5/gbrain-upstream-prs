...
description: "Extract structured information from long documents in two passes: a fast skim for structure and entities, then a deep pass only where it pays. Use for long transcripts, reports, and email threads"
triggers:
  - "two tier extraction"
  - "long transcripts"
  - "email threads"
  - "extract structured information from long documents in two passes"
  - "a fast skim for structure and entities"
  - "then a deep pass only where it pays"
---
...
>
> **Convention:** see the `model-routing` skill —
> this skill uses the knowledge base's tier vocabulary (`utility` / `reasoning` / `deep`).
> Resolve tiers through `the knowledge base models`; never hardcode a model ID.
>
...
the `regex-discipline` skill for
pattern hygiene):
...
```bash
the knowledge base models                          # current tier → model table
the knowledge base config set models.tier.deep opus   # example: pin the escalation tier
```
...
- **Hardcoding model IDs.** Tiers resolve through the model-routing
  convention (`the knowledge base models`); a hardcoded ID rots and silently breaks.
