---
name: enrich
description: "Tiered protocol for building and updating person and company pages: compiled truth, timeline, cross-links. Use when a new entity is mentioned or an existing profile is thin"
triggers:
  - "enrich"
  - "new entity is mentioned"
  - "existing profile is thin"
  - "tiered protocol for building and updating person and company pages"
  - "compiled truth"
  - "timeline"
---
...
> **Filing rule:** Read the `filing-rules` skill before creating any new page.

> **Convention:** See `skills/conventions/quality.md` for Iron Law back-linking.

Every mention of a person or company with a brain page MUST create a back-link
FROM that entity's page TO the page mentioning them. An unlinked mention is a
broken brain. See the `filing-rules` skill for format.
...
For each entity:
- `(look this up in your connected knowledge base, if you have one)` -- does a page already exist?
...
- `(look this up in your connected knowledge base, if you have one)` and `(look this up in your connected knowledge base, if you have one)`
...
Store raw API responses via `put_raw_data` in the knowledge base:
```json
{
  "source": "crustdata",
...
1. Check notability gate (see the `filing-rules` skill)
...
Timeline entries still need explicit `the knowledge base timeline-add` calls.
...
- Store/update a page in the knowledge base (put_page)
- Add a timeline entry in the knowledge base (add_timeline_entry)
- List pages in the knowledge base by type (list_pages)
- Store raw API data in the knowledge base (put_raw_data)
- Retrieve raw data from the knowledge base (get_raw_data)
- Link entities in the knowledge base (add_link)
