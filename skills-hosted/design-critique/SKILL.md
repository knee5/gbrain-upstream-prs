...
description: "Designer's-eye critique of a live interface: first impression, design-system extraction, page-by-page audit, interaction flow, and cross-page consistency, ending in a triaged report"
triggers:
  - "design critique"
  - "designer's-eye critique of a live interface"
  - "first impression"
...
If the output is non-empty (working tree is dirty), **STOP** and use a multiple-choice question to the user:
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. a multiple-choice question to the user: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed."
...
**Project-scoped:**
```bash
eval "$(the local helper CLI (unavailable here) 2>/dev/null)" && mkdir -p the local state folder (unavailable here)$SLUG
```
Write to: `the local state folder (unavailable here)`
...
```
the local state folder (unavailable here)$SLUG/designs/design-audit-{YYYYMMDD}/
...
eval "$(the local helper CLI (unavailable here) 2>/dev/null)" && mkdir -p the local state folder (unavailable here)$SLUG
```
Write a one-line summary to `the local state folder (unavailable here)` with a pointer to the full report in `$REPORT_DIR`.
...
## Phase 11: the project TODO list Update

If the repo has a `the project TODO list`:

1. **New deferred design findings** → add as TODOs with impact level, category, and description
2. **Fixed findings that were in the project TODO list** → annotate with "Fixed by /design-review on {branch}, {date}"

---

## Additional Rules (design-review specific)

11. **Clean working tree required.** If dirty, use a multiple-choice question to the user to offer commit/stash/abort before proceeding.
