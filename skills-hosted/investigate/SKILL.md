---
name: investigate
description: "Root-cause debugging in five phases: investigate, pattern analysis, hypothesis testing, implementation, verification. Use for any bug where the cause is not obvious. Bans guess-and-patch"
triggers:
  - "investigate"
  - "bug where the cause is not obvious"
  - "root-cause debugging in five phases"
  - "pattern analysis"
  - "hypothesis testing"
  - "implementation"
---
...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via a multiple-choice question to the user.
...
```bash
eval "$(the local helper CLI (unavailable here))"
...
Also check:
- `the project TODO list` for related known issues
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use a multiple-choice question to the user:
   ```
...
5. **If the fix touches >5 files:** Use a multiple-choice question to the user to flag the blast radius:
   ```
...
Related:         [the project TODO list items, prior bugs in same area, architectural notes]
...
```bash
the local helper CLI (unavailable here) '{"skill":"investigate","type":"investigation","key":"ROOT_CAUSE_KEY","insight":"ROOT_CAUSE_SUMMARY","confidence":9,"source":"observed","files":["affected/file1.ts","affected/file2.ts"]}'
```
...
- **If fix touches >5 files → a multiple-choice question to the user** about blast radius before proceeding.
- **Completion status:**
