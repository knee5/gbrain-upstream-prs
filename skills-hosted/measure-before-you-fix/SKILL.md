...
description: "Refuse to optimize or repair anything before you have a baseline measurement and a named metric. Use whenever someone proposes a performance, cost, or quality fix"
triggers:
  - "measure before you fix"
  - "someone proposes a performance"
  - "quality fix"
---
...
On the knowledge base surfaces this covers: `the knowledge base's health check` staleness checks (e.g. sync
...
- The alert's threshold is compared against the authoritative one
  (`the knowledge base's health check`'s warn/fail lines) before the system is declared unhealthy.
...
2. **Time the suspect step directly.** Isolate the smallest unit that the alert
   ...
   ```bash
   time the knowledge base sync --source source-a --no-embed
   ```
   ...
   Cross-check state with `the knowledge base sources status` (per-source sync lag).
...
4. **Check the alert threshold against the authoritative one.** Before
   ...
   what "bad" means. `the knowledge base's health check`'s sync-freshness check defaults to
...
- The alert says "no recovery in flight" — verify whether recovery is in fact
  ...
  `the knowledge base jobs list` for queued work).
...
- **Raising a timeout to fix a stall.** If the step is genuinely hung, a bigger
  ...
  "is wedged" — they have opposite fixes. (the knowledge base's sync stall watchdog makes
...
- **Triple-wrong diagnosis on a freshness alert.** A cron monitor paged
  ...
  `time the knowledge base sync --source source-a --no-embed` finished in single-digit
  seconds with "Already up to date" (same for `source-b`), and
  `the knowledge base sources status` showed every source synced that morning. Every
  theory died at once. Actual cause: the monitor alerted at its
  act-threshold, hours below `the knowledge base's health check`'s authoritative warn line. The
