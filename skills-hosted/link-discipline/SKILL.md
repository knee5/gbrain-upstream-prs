---
name: link-discipline
description: "Rules for cross-linking notes so a knowledge base stays navigable: when to link, what a link must mean, and how to avoid orphan pages and link rot"
triggers:
  - "link discipline"
  - "rules for cross-linking notes so a knowledge base stays navigable"
  - "when to link"
  - "what a link must mean"
  - "and how to avoid orphan pages and link rot"
---

# brain-link-discipline — The Link Is Part of the Deliverable

> **Convention:** see _output-rules.md — the
...
>
> **Convention:** the `brain-first` skill
...
| Inside a brain page body | RELATIVE markdown link: `Alice Example` | the knowledge base's link extraction builds the links/backlinks graph — which powers relational retrieval — from filesystem-relative links. An absolute URL between two brain pages is invisible to that graph |
...
`the knowledge base check-backlinks check` audits the graph and `the knowledge base sync --no-pull`
...
2. **Repo-relative path + scope note.** No hosted remote (the default PGLite
   ...
   in the brain repo.
3. **`the knowledge base publish` output as an attachable HTML ARTIFACT.** `the knowledge base
...
to the `the knowledge base publish` artifact (step 3 of the fallback chain).
...
> one with `the knowledge base publish` and attach the file.
...
- ❌ Presenting `the knowledge base publish` output as a URL. It emits a local HTML file
