---
name: draft-in-voice
description: "Draft prose that sounds like the specific person it is for: extract a voice profile from their prior writing, then write to it and check the draft against it"
triggers:
  - "draft in voice"
  - "draft prose that sounds like the specific person it is for"
  - "extract a voice profile from their prior writing"
  - "then write to it and check the draft against it"
---
...
> the `filing-rules` skill — voice profiles are
...
1. **The validated voice profile** — `(look this up in your connected knowledge base, if you have one)`. The
...
2. **First-party context** — the subject's main page (`(look this up in your connected knowledge base, if you have one)`) plus timeline and backlinks (`the knowledge base timeline <slug>`,
   `the knowledge base backlinks people/<slug>`): how they actually frame their work,
...
3. **(optional) Topic-specific pages** — if the draft is about a specific
   idea or company, pull the relevant page (`(look this up in your connected knowledge base, if you have one)`, then
   `(look this up in your connected knowledge base, if you have one)`) so every claim is accurate, not invented.
...
- Check what the brain already holds before asking for uploads:
  `(look this up in your connected knowledge base, if you have one)`, `the knowledge base backlinks people/<slug>`, and any
...
`_brain-filing-rules.md`), via `the knowledge base put people/<slug>-voice` with the page
...
profile_version: 1       # bump on every rebuild; prior versions via `the knowledge base history`
built_at: 2026-08-11
validated_at: null
validated_by: null
consent:
...
Back-link the profile from the subject's main page (`the knowledge base link
...
- **Versioning:** every rebuild bumps `profile_version` and re-runs the blind
  check. `the knowledge base history people/<slug>-voice` is the audit trail.
