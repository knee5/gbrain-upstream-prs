...
description: "Verify academic and research citations: confirm the paper exists, the claim matches the abstract, and the finding has not been superseded or retracted. Use when a document cites studies"
triggers:
  - "academic verify"
  - "document cites studies"
  - "verify academic and research citations"
  - "confirm the paper exists"
  - "the claim matches the abstract"
  - "and the finding has not been superseded or retracted"
---
...
- Not a brain-only lookup — that's `(look this up in your connected knowledge base, if you have one)`.
...
by the `perplexity-research` skill. academic-verify's
...
Step 2: Brain-first lookup
  the knowledge base query "<paper title> OR <author name> OR <claim keywords>"
