...
description: "Turn scattered mentions of an idea across many sources into one durable concept page with a definition, the strongest supporting evidence, open questions, and dissenting views"
triggers:
  - "concept synthesis"
---
...
The skill is markdown agent instructions. The agent uses the knowledge base's
existing operations + LLM passes:

```bash
# 1. List all concept pages
the knowledge base query "type:concept" --limit 10000 --json
...
> **Convention:** see the `test-before-bulk` skill
...
not hardcode a similarity threshold: `(look this up in your connected knowledge base, if you have one)` returns hybrid
...
the knowledge base search "concept title + first paragraph" --limit 10
...
the knowledge base get concepts/absorbed-stub
the knowledge base put concepts/_merged/cluster-name/absorbed-stub

# 3. Grow the canonical head: merge_count, independent_sources,
#    backlinks, and the ## Facets section
the knowledge base put concepts/canonical-slug

# 4. Soft-delete the absorbed original (restorable until purge)
the knowledge base delete concepts/absorbed-stub

# Undo paths: the knowledge base restore <slug> (within the purge window),
# the _merged/ copy (survives purge), and per-page version history:
the knowledge base history concepts/canonical-slug
the knowledge base revert concepts/canonical-slug <version_id>
```
...
- Run `(look this up in your connected knowledge base, if you have one)` and spot-check the count reduction.
