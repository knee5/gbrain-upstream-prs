...
description: "Build a durable multi-source research document on a topic: plan the questions, gather and archive sources, synthesize, and keep it updatable rather than one-shot"
triggers:
  - "research compendium"
  - "build a durable multi-source research document on a topic"
  - "plan the questions"
  - "gather and archive sources"
  - "synthesize"
  - "and keep it updatable rather than one-shot"
---

# research-compendium — Archive Everything, Summarize 1:1, Synthesize Once

> **Convention:** see the `brain-first` skill
...
>
> **Convention:** see the `quality` skill for
> citation rules, quote fidelity, and back-link enforcement.
>
> **Convention:** see the `filing-rules` skill —
...
> **Convention:** see the `untrusted-content` skill
...
via `the knowledge base recall` and search. An injected instruction archived today
...
- Binary originals (PDFs, images) attach to their source page via
  `the knowledge base files upload-raw <file> --page research/<topic-slug>/sources/NN-<source-slug>`.
- Wire links in both directions as you go: `the knowledge base link` each source ↔ index
  and summary ↔ compendium, and run `the knowledge base check-backlinks check` at close-out.
...
`archived`/`summarized`/`mirrored` booleans, each `the knowledge base lsd` pass (seed
...
| `compendium+` | Grounded | Full-text primaries (papers/RFCs/primary blog posts) acquired and each summary re-read *against the driving question* (mechanism + tension, not generic recap). One formal `the knowledge base lsd` pass. Claims ledger + fact-check gate turn ON. | moderate |
| `compendium++` | Deep | Books enter (summary-tier, with verbatim quotes). **Counter-canon hunt**: acquire the best critiques/recantations of each pillar. 2-3 `the knowledge base lsd` passes, cross-modal eval per pass. | higher |
| `compendium+++` | Saturated | Full book-mirrors on the 1-2 most central books (via the `book-mirror` skill, user opt-in). Cross-axis mapping as its own section. `the knowledge base lsd` repeated until new passes stop surfacing survivors (log the saturation point). | high |
...
- **Cheap tier first.** Free/verifiable acquisition before expensive; surface
  ...
  the `test-before-bulk` skill before
  any bulk acquisition run.
- **`the knowledge base lsd` passes are real runs** on the archived corpus (not in-head
  synthesis): `the knowledge base lsd "<the driving question>" --save --max-cost 5`. Seed
...
Brain first: `(look this up in your connected knowledge base, if you have one)` and `(look this up in your connected knowledge base, if you have one)` — the brain
may already hold part of the corpus. Then the open web: route web research
through the `perplexity-research` skill and whatever
...
hooks are what make later `the knowledge base lsd` passes productive — pre-wired
...
the `fact-check` skill skill over the compendium + ledger before
...
the `cross-modal-review` skill (or `the knowledge base eval
...
central ones via the `book-mirror` skill (deeper, real cost
...
the `minion-orchestrator` skill. The sub-task prompt
...
render the compendium via the `brain-pdf` skill or publish a
shareable HTML page with `the knowledge base publish`. Run the fact-check gate BEFORE
...
- **the `data-research` skill** — structured data into
...
- **the `perplexity-research` skill** — the delta pass:
  ...
  web lookups.
- **the `academic-verify` skill** — ONE academic claim
...
- **the `strategic-reading` skill** — ONE source read
  ...
  reference asset.
- **the `concept-synthesis` skill** — synthesizes what is
...
- Output written under the directories listed in `writes_to:`; links wired in
  both directions and validated with `the knowledge base check-backlinks check`.
...
itself when routed — nothing in the the knowledge base runtime mechanically blocks an
