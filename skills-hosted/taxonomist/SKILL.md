---
name: taxonomist
description: "Design and maintain the tag and category system for a knowledge base: keep the vocabulary small, merge near-duplicates, and prevent tag sprawl"
triggers:
  - "taxonomist"
  - "design and maintain the tag and category system for a knowledge base"
  - "keep the vocabulary small"
  - "merge near-duplicates"
  - "and prevent tag sprawl"
---
...
driven by `the knowledge base schema show --json`. This means:
- A user who runs `the knowledge base schema use the knowledge base-recommended` gets the full
  ...
  plus all the knowledge base-base types).
- A user who authored a custom pack via `the knowledge base schema init` + edit gets
  filing recommendations based on THEIR taxonomy, not the knowledge base's defaults.
...
```bash
the knowledge base schema show --json | jq '.page_types[] | select(.primitive == "entity")'
```
...
The `the knowledge base-recommended` pack treats books as `media/books/<category>/<slug>.md`
...
walk it from `the knowledge base schema show --json` instead of hardcoding here.
...
the knowledge base schema review-orphans --json

# What's the overall health?
the knowledge base's health check --json | jq '.checks[] | select(.name == "schema_pack_consistency")'
```
...
**Suggested next step:** `the knowledge base schema detect --source <source_id>` then
`the knowledge base schema review-candidates`.
```

## Anti-Patterns

- **Hardcoded directory table in this skill.** Every decision goes through
  `the knowledge base schema show --json`. v0.39+ broke the old hardcoded table on
  purpose so users on `the knowledge base-recommended` or custom packs get the right
...
- **Auto-applying a `the knowledge base schema review-candidates --apply` decision.**
...
- **Never hardcode a directory table in this skill.** Every decision goes
  through `the knowledge base schema show --json`. The active pack is canonical.
...
### v1.0.0 — the knowledge base v0.39.0.0
- Initial port from upstream OpenClaw. Genericized — no references to
  private fork names per the project instructions file privacy rules.
- Hardcoded directory table REMOVED. Every decision now reads the active
  schema pack via `the knowledge base schema show --json`. Single source of truth.
- Book taxonomy moved from skill-text to the `the knowledge base-recommended` pack's
  media/books/ branch (see `src/core/schema-pack/basethe knowledge base-recommended.yaml`).
