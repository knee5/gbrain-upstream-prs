---
name: fact-check
description: "Systematically verify claims in a document: classify each claim, find primary sources, rate confidence, and mark what could not be verified. Use before publishing anything factual"
triggers:
  - "fact check"
  - "publishing anything factual"
  - "systematically verify claims in a document"
  - "classify each claim"
...
```
the knowledge base search "<entity>"
```
...
(the `brain-first` skill) applied to
verification.
...
|---|---|
| `(look this up in your connected knowledge base, if you have one)` (expansion/synthesis) | `(look this up in your connected knowledge base, if you have one)` + `(look this up in your connected knowledge base, if you have one)` to read the page itself |
| `(look this up in your connected knowledge base, if you have one)` (hybrid retrieval) | `the knowledge base graph-query <slug> --type <edge>` or `the knowledge base backlinks <slug>` |
| graph traversal (`the knowledge base graph` / `graph-query`) | direct page read (`(look this up in your connected knowledge base, if you have one)`) — does the page actually assert this? |
...
```
the knowledge base graph-query alice-example --type founded
the knowledge base graph-query fund-a --type invested_in --direction out
```
...
- **the `academic-verify` skill** — DEPTH trace of ONE
...
- **the `citation-fixer` skill** — citation FORMAT
...
- **the `cross-modal-review` skill** — second-MODEL
...
- **the `perplexity-research` skill** — open-ended
