...
`test/e2e/serve-http-oauth.test.ts` additionally pins confidential POST/Basic revocation, public-client SDK fallthrough, malformed/mixed authentication rejection, cross-client isolation, unknown-token opacity, metadata auth methods, no-store responses, strict post-revoke `401`, retryable backend `503` semantics, registration-time OAuth slug-write namespaces, and destructive-tool catalogs.
...
- `test/oauth.test.ts` — OAuth 2.1 provider: register, getClient, `client_credentials` grant exchange, `authorization_code` flow with PKCE challenge/verifier, refresh token rotation, `verifyAccessToken` with both OAuth + legacy `access_tokens` fallback, registration-time `bound_slug_prefixes` round-trip into `AuthInfo.writeSlugPrefixes`, `revokeToken`, `sweepExpiredTokens`; contract test asserting `scope` + `localOnly` annotations on all operations; `coerceTimestamp` unit cases (null/undefined/string/number/throw-on-NaN); NULL-`expires_at`-as-expired contract for both refresh + access token paths; cascade-delete contract asserting `revoke-client` purges `oauth_tokens` + `oauth_codes` via FK CASCADE; cross-client isolation (wrong-client attempt MUST reject AND rightful owner MUST still succeed atomically afterward); empty-string `redirect_uri` bypass guard; PKCE DCR public-client gate (`token_endpoint_auth_method: "none"` returns no `client_secret`, default `client_secret_post` clients get the one-time-reveal secret, `getClient` NULL→undefined normalization, full PKCE `/authorize` → `/token` round-trip against a public client).
- `test/mcp-stdio-identity.test.ts` — stdio entrypoint identity boundary: `buildStdioDispatchOpts` preserves `transport:'stdio'` through `buildOperationContext`, keeps `remote:true`, grants no OAuth scopes, and makes auth-less `whoami` return the stdio shape instead of `unknown_transport`.
- `test/mcp-tool-defs.test.ts` — centralized MCP schema + annotation mapping: byte-stable ParamDef recursion, complete logical-read annotations, the explicit `search_by_image` open-world exception, no guessed annotations on mutating/privileged tools, and scope-hierarchy filtering for read/write/admin catalogs, including admin-only `delete_page` and `restore_page`.
- `test/operations-allow-list.test.ts` — fail-closed write namespace policy for subagent and OAuth callers: every routine remote OAuth slug-targeting path (`put_page`, tags, both link endpoints, timeline, raw data, every ingest-log page, ontology) requires a matching registration-time prefix, with allowed/outside/unbound invocation coverage; explicit admin OAuth and trusted local callers retain operator behavior.
- `test/serve-http-ingest-slug.test.ts` — pure `/ingest` slug resolution contract: a routine OAuth writer defaults beneath its first wildcard namespace, explicit in/out-of-lane slugs are accepted/rejected, missing bindings fail closed, exact-only grants require `X-Gbrain-Slug`, malformed slugs never reach the queue, and admin retains the legacy global-inbox default.
- `test/ingestion/put-page-write-through.test.ts` — `put_page` dual-sink boundary: trusted local callers retain atomic filesystem mirroring rendered from the persisted DB provenance, while remote/unset-trust callers persist DB content + provenance and return `write_through.skipped='remote'` without touching the configured repo path.
