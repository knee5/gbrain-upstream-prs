...
`test/e2e/serve-http-oauth.test.ts` additionally pins confidential POST/Basic revocation, public-client SDK fallthrough, malformed/mixed authentication rejection, cross-client isolation, unknown-token opacity, metadata auth methods, no-store responses, strict post-revoke `401`, retryable backend `503` semantics, registration-time OAuth page-write namespaces, and admin-only delete/restore catalogs.
...
- `test/operations-allow-list.test.ts` — fail-closed write namespace policy for subagent and OAuth callers: routine remote OAuth `put_page` requires a matching registration-time prefix, while explicit admin OAuth and trusted local callers retain operator behavior.
