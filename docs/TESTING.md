...
- `test/mcp-tool-defs.test.ts` — centralized MCP schema + annotation mapping: byte-stable ParamDef recursion, complete logical-read annotations, the explicit `search_by_image` open-world exception, no guessed annotations on mutating/privileged tools, and scope-hierarchy filtering for read/write/admin catalogs.
- `test/ingestion/put-page-write-through.test.ts` — `put_page` dual-sink boundary: trusted local callers retain atomic filesystem mirroring, while remote/unset-trust callers persist DB content + provenance and return `write_through.skipped='remote'` without touching the configured repo path.
